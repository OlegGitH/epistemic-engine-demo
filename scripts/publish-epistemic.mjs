...
const suffix=(process.env.EPISTEMIC_EXTERNAL_ID_SUFFIX||"").trim().replace(/[^a-zA-Z0-9._-]+/g,"-");
const externalID=`github-${repository}-${runID}-${attempt}${suffix?`-${suffix}`:""}`;
const response=await fetch(`${endpoint}/v1/ingest`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({external_id:externalID,ai_system_id:process.env.EPISTEMIC_AI_SYSTEM_ID||"",policy_version:"epistemic.dev/v0.1",context:{repository,commit_sha:process.env.GITHUB_SHA||"",branch:process.env.GITHUB_REF_NAME||"",workflow:process.env.GITHUB_WORKFLOW||"Food Lens CI",run_url:`${process.env.GITHUB_SERVER_URL||"https://github.com"}/${repository}/actions/runs/${runID}`},report,certificate})});
